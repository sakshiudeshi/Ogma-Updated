...
base_src = os.getcwd()
paths = ["../Plots/master-error-start-time/", "../Plots/master-non-error-start-time//"]
for path in paths:
    if not os.path.exists(path):
        os.makedirs(path)

folders = ["MasterData Positive Start", "MasterData Negative Start"]
...
for folder in folders:
    ...
    # src = "DataFiles/MasterData/"+ folder_type + "/"
        ...
        fig.suptitle('Timing of ' + folder_type + '\n (Jaccard Index < ' + jacc_thresh_val + ')')
        plt.xticks(index, labels_timing)
        plt.ylabel('Time (Minutes)')
        if folder == "MasterData Positive Start":
            os.chdir("../../../Plots/master-error-start-time")
        else:
            os.chdir("../../../Plots/master-non-error-start-time")
